...
def _row_to_patient(row: sqlite3.Row) -> Patient:
    ...
    return Patient(
        ...
        diagnosis=_present(row["NombreDiagnostico"]) or MISSING,
        allergies=row["Alergias"] or MISSING,
        vitals=Vitals(
            bp=_present(row["TensionArterial"]) or MISSING,
            hr=_with_unit(row["FrecuenciaCardiaca"], " lpm"),
            temp=_with_unit(row["Temperatura"], "°C"),
...
def _bed_info(conn: sqlite3.Connection, bed_id: str, current_oid: int | None = None) -> sqlite3.Row:
    bed = conn.execute(
...
def create_patient(conn: sqlite3.Connection, data: PatientCreate) -> Patient:
    now = now_str()
    bed = _bed_info(conn, data.bed_id) if data.bed_id else None
    level = data.triage_level or 4
    ...
    try:
        ...
        conn.execute(
            ...
            (triage_id, now, data.symptoms or MISSING, vitals.bp or MISSING, _digits(vitals.hr),
             MISSING, _digits(vitals.temp), patient_id, str(level),
...
def update_patient(conn: sqlite3.Connection, patient_id: str, data: PatientUpdate) -> Patient:
    ...
    try:
        ...
        if data.bed_id and data.bed_id != current["CodigoCama"] and not discharging:
            bed = _bed_info(conn, data.bed_id, current_oid=oid)
            conn.execute(
...
def _update_triage(
    ...
    if not triage_id:
        triage_id = _next_id(conn, "Triage", "OidTriage")
        level = data.triage_level or current["NivelTriage"] or 4
        conn.execute(
            "INSERT INTO Triage (OidTriage, FechaTriage, MotivoConsulta, TensionArterial, "
            "FrecuenciaCardiaca, FrecuenciaRespiratoria, Temperatura, IdPaciente2, CodigoTriage, "
            "ClasificacionTriage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (triage_id, now_str(), MISSING, vitals.bp or MISSING, _digits(vitals.hr), MISSING,
             _digits(vitals.temp), current["IdPaciente"], str(level),
             f"REGISTRO APP - TRIAGE {level}"),
        )
    ...
    if data.vitals:
        updates = {
            ...
            "FrecuenciaCardiaca": _digits(vitals.hr) if vitals.hr else None,
            "Temperatura": _digits(vitals.temp) if vitals.temp else None,
        }
