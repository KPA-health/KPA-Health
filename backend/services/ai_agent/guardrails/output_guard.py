...
def looks_like_reasoning(text: str) -> bool:
    """True si el texto parece razonamiento interno del modelo en lugar de una respuesta."""
...
def contains_forbidden_terms(text: str) -> bool:
    """True si quedan términos técnicos que el usuario de negocio no debe ver."""
...
def sanitize_text(text: str) -> str:
    """Enmascara documentos y reemplaza jerga técnica por lenguaje del negocio."""
...
def _format_value(value: Any) -> str:
    """Formato numérico colombiano: miles con punto y decimales con coma."""
    if value is None:
        return "sin dato"
...
def _descriptive_column(labels: list[str], rows: list[list[Any]]) -> int:
    """Índice de la columna más descriptiva (nombre de medicamento, servicio...) para los ejemplos."""
...
def _as_number(token: str) -> float | None:
    """Convierte '1.305,5' o '12.5' a float; None si no es un número."""
    token = token.strip()
