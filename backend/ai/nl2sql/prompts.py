...
- Rol y alcance explícitos (solo operación del hospital) + clasificación obligatoria
  de la pregunta en una categoría (hospital, fuera_de_alcance, idioma_no_soportado,
  datos_personales) -> el modelo decide ANTES de generar SQL.
...
from backend.ai.nl2sql.catalog import FEW_SHOT_EXAMPLES, SCHEMA_DESCRIPTION
...
{"categoria": "<hospital|fuera_de_alcance|idioma_no_soportado|datos_personales>",
...
- "fuera_de_alcance": cualquier otro tema (clima, gobierno, política, deportes, noticias, economía, recetas,
...
- "idioma_no_soportado": la pregunta NO está escrita en español. "sql" es null.
- "datos_personales": piden nombres, apellidos, documentos de identidad, teléfonos, direcciones o correos de
...
NL2SQL_SCHEMA = {
    "type": "object",
    "properties": {
        "categoria": {"type": "string", "enum": ["hospital", "fuera_de_alcance", "idioma_no_soportado", "datos_personales"]},
        "sql": {"type": ["string", "null"]},
        "explanation": {"type": "string"},
    },
    "required": ["categoria", "sql", "explanation"],
}
SUMMARY_SCHEMA = {
    "type": "object",
...
def build_system_prompt() -> str:
    examples = "\n\n".join(
        f"Pregunta: {question}\n"
        + json.dumps(
            {"categoria": category, "sql": None if sql == "NULL" else sql, "explanation": explanation},
            ensure_ascii=False,
        )
...
Responde SOLO con un objeto JSON: {"respuesta": "<tu respuesta en español>"}
""".strip()
