...
MediPulse.ApiClient = {
  ...
  /** Petición HTTP real. Lanza errores con `network` (sin respuesta) o `status` (HTTP). */
  ...
  buildMockDashboard(params, { patients, rooms, pharmacy }) {
    ...
    return {
      ...
      wards: rooms.map(w => {
        ...
        const libres = w.beds.filter(b => b.status === 'Libre').length;
        return { wing: w.wing, total, occupied: occ, free: libres, unavailable: total - occ - libres, occupancyPct: total ? Math.round((occ / total) * 100) : 0 };
      }),
