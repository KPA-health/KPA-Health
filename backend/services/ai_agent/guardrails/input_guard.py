...
class GuardDecision:
    """Resultado de un eslabón: si la pregunta pasa y, si no, la categoría y el mensaje fijo."""
    allowed: bool
    category: str
...
def normalize(text: str) -> str:
    """Minúsculas, sin tildes y con espacios simples: las regex no dependen de la ortografía."""
...
class InputGuard(ABC):
    """Eslabón de la cadena de filtros de entrada."""
...
# El orden importa: lo más barato y lo más grave primero (inyección antes que saludos).
...
def evaluate_input(question: str, chain: tuple[InputGuard, ...] = DEFAULT_CHAIN) -> GuardDecision:
    """Recorre la cadena; el primer eslabón que bloquea decide. Si nadie bloquea, pasa al LLM."""
