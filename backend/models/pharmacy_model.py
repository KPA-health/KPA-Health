"""
Modelo de farmacia (capa Modelo): consultas y reglas de negocio de /pharmacy.
...
# Lo más urgente primero: críticos, luego bajos, y dentro de cada grupo menos días de cobertura
_ORDER_BY_URGENCY = (
...
def _to_item(row: sqlite3.Row) -> PharmacyItem:
    """Fila de InventarioFarmacia -> DTO PharmacyItem."""
    return PharmacyItem(
...
def list_items(
    ...
    """Inventario filtrado; devuelve (ítems, total sin límite)."""
    where, params = [], []
    ...
    sql = f"SELECT * FROM InventarioFarmacia{clause} {_ORDER_BY_URGENCY}"
    if limit:
        sql += " LIMIT ?"
        params.append(limit)
...
def get_item(conn: sqlite3.Connection, item_id: str) -> PharmacyItem:
    """Un medicamento/insumo por su código o NotFoundError."""
    row = conn.execute(
...
def update_item(conn: sqlite3.Connection, item_id: str, data: PharmacyUpdate) -> PharmacyItem:
    """Actualiza stock/lote/vencimiento; el estado (Crítico/Bajo/Adecuado) lo recalcula SQLite."""
