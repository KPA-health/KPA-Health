"""
Modelo de camas (capa Modelo): consultas y reglas de negocio de /rooms.
...
# Valores que significan "quitar el estado manual y volver al calculado desde el HIS"
...
def _to_bed(row: sqlite3.Row) -> Bed:
    """Fila de EstadoCamas -> DTO Bed (el ocupante solo aparece si la cama está ocupada)."""
...
def list_rooms(conn: sqlite3.Connection) -> list[Wing]:
    """Camas agrupadas por servicio y subgrupo ("alas"), el formato que pinta la SPA."""
...
def update_bed_status(conn: sqlite3.Connection, data: BedStatusUpdate) -> None:
    """Fija un estado manual (desinfección, mantenimiento...) o lo retira con 'Auto'."""
    exists = conn.execute(
